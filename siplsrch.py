...
def read_singlepulse_files(infiles, threshold, T_start, T_end, lt, ut, ld, ud, ls, us, lw, uw, sm):
	...
	for ii, infile in enumerate(infiles):
		...
		if os.stat(infile)[6]:
			try:
				cands = Num.loadtxt(infile)
				if (len(cands.shape) == 1 and ld <= cands[0] <= ud and lt <=cands[2] <= ut and ls <= cands[1] <= us and lw <= cands[4] <= uw):
					DMs.append(cands[0])
					candlist.append(cands[1])
					ime.append(cands[2])
					width.append(cands[4])
					noofpulse.append(int(1))
				
				else:
					for x in cands:
						if (ld <= x[0] <= ud and lt <=x[2] <= ut and ls <= x[1] <= us and lw <= x[4] <= uw):
							DMs.append(x[0])
							candlist.append(x[1])
							ime.append(x[2])
							width.append(x[4])
...
def main():
	...
	parser.add_option("-p", "--noplot", action="store_false", dest="makeplot",
			default=False, help="Look for pulses but do not generate a plot")
	...
	if args[0].endswith(".singlepulse") or args[0].endswith(".ascii"): # For plotting
	...
	if not dosearch:
		DMs, candlist, ime, width, noofpulse = \
            		read_singlepulse_files(args, opts.threshold, opts.T_start, opts.T_end, lt, ut, ld, ud, ls, us, lw, uw, sm) 
		...
		for i, ii in enumerate(DM_sorted):
			if len(snrz[i]) >= 1000: # If length of List of SNR corresponding to a particular DM is greater than 1000
				...
				outfile = open(DMstr +'.singlepulse', mode = 'w') # opening Newly created ".singlepulse" file
             ...
             # Split the imeseries into chunks for detrending
				...
				if nn_last == 100:
					...
					imeseries.shape = (numblocks, detrendlen) # imeseries is reshaped 
                                                        		   # i.e each subpart is of length 100 bins
            ...
            # will split off the chunks with very low and very high stds
				...
				if (opts.badblocks): # it is for removing the bad part of the imeseries whcich is not of our use
					...
					stds[bad_blocks] = median_stds # replacing values of items in list stds with indexes stored in bad-blocks
                                                # by median_stds i.e. stds[with index in bad_blocks] = median_stds
				else:
					bad_blocks = []
            ...
            # because of the overlaps for the convolutions
                ...
                # Take care of beginning and end of file overlap issues 
					if (chunknum==0):  # Beginning of file 
						...
						tchunk = Num.asarray(td[int(loind):int(hiind)])
...
# signal-to-noise pulse				
				...
				bdm_candlist = []
				for h in ind:
					bdm_candlist.append(candidate(d[h], snr[h], t[h], 1, w[h]))
...
# Write the pulses to an ASCII output file
				if len(dm_candlist):
					outfile.write("# DM	   Sigma     ime (s)	      Sample Width\n")
...
# Write the pulses to an ASCII output file
		...
		print ("Percentage of garbage data is " + str(p) + "%.")
		end = time.time()
	...
	if (op): # For Plotting
		...
		J = []
		S = []
		l = sorted(zip(candlist, DMs, ime, width, noofpulse), reverse = True) # Making A dictionary which is sorted according
                                                                              # to SNR values
		...
		ax1.set_title("Width vs DM Scatter (SNR in color)") # (addition 0)
		ax1.set_ylabel("Width (SNR)")
		ax1.set_xlabel('DM')
		ax1.axis([min(D)-5,max(D)+5,min(W)-0.2,max(W)+0.5])
		cc = ax2.scatter(D, W, c= J, s = J)
		ax2.set_xlabel("DM")
		...
		ax3.plot(DMs, candlist, "o", markersize = 0.5)
		ax3.set_ylabel("SNR")
		ax3.set_xlabel("DM")
		ax3.set_title("SNR vs DM Scatter")
		plo.suptitle("First Color Scale --> No. of Pulse and Second Color Scale --> SNR")
		plo.colorbar(bb)
		plo.colorbar(cc)
		end = time.time()
        	print "Run Time of code in seconds is " + str(end - start)
